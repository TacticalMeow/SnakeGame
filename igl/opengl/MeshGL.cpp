...
IGL_INLINE void igl::opengl::MeshGL::bind_skybox()
{
    glBindVertexArray(vao_mesh);
    glUseProgram(shader_skybox);
    bind_vertex_attrib_array(shader_skybox, "position", vbo_V, V_vbo, dirty & MeshGL::DIRTY_POSITION);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_F);
    if (dirty & MeshGL::DIRTY_FACE)
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned) * F_vbo.size(), F_vbo.data(), GL_DYNAMIC_DRAW);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_CUBE_MAP, vbo_tex);
...
{
    if(is_cubemap) //we bind the cubemap instead
    {
        bind_skybox();
    }
    else //we just bind a regular mesh
    {
        glBindVertexArray(vao_mesh);
...
IGL_INLINE void igl::opengl::MeshGL::draw_skybox()
{
...
R"(#version 150
  ...
  //Compile the cubemap shader - Eden Mansdorf
  create_shader_program(
