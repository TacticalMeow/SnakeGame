...
//    grid_texture();
  ...
  //Initialize AABB for collision detection
  tree.init(V, F);
  lastTree = tree;
}
...
bool igl::opengl::ViewerData::load_texture_as_cubemap(const std::string& textures_directory)
{
    meshgl.init();
    glBindTexture(GL_TEXTURE_CUBE_MAP, meshgl.vbo_tex);
    std::vector<std::string> faces =
    {
            textures_directory +"/right.jpg",
            textures_directory +"/left.jpg",
            textures_directory +"/top.jpg",
            textures_directory +"/bottom.jpg",
            textures_directory +"/front.jpg",
            textures_directory +"/back.jpg"
    };
    //for (auto& face : faces)
    //{
    //    face = texture_filename + face;
    //}
    ...
    meshgl.is_cubemap = true;
    return true;
}

//Just adds a big bounding box to the mesh
