...
IGL_INLINE void Renderer::draw( GLFWwindow* window)
{
	using namespace std;
	using namespace Eigen;
	if (((SandBox*)scn)->is_first_person_view)
	{
		first_person_view();
	}
	else
...
void Renderer::handle_dirty_datalist()
{
	if (((SandBox*)scn)->dirty_data_list)
	{
		for (size_t i = 0; i < scn->data_list.size(); i++)
		{
			if (scn->data_list[i].is_game_object && scn->data_list[i].dirty_object)
			{
				for (auto& core : core_list)
				{
				...
				scn->data_list[i].dirty_object = false;
			}
		}
		((SandBox*)scn)->dirty_data_list = false;
	}
}
...
void Renderer::MouseProcessing(int button)
{
	...
	else
	{
		if (button == 1)
		{
			...
			//scn->RotateInSystem(Eigen::Vector3d(1, 0, 0), yToMove);

			//scn->RotateInSystem(Eigen::Vector3d(0, 1, 0), xToMove);
		}
		else
		{
			...
			RotateCamera(yrel / 1000.0, xrel / 1000.0);
...
void Renderer::RotateCameraCenter(float amtX, float amtY)
{
	core().camera_center = RotateAroundPoint(Vector3f(1, 0, 0), core().camera_eye, amtX) *
						   RotateAroundPoint(Vector3f(0, 1, 0), core().camera_eye, amtY) *
						   core().camera_center;
}
Eigen::Affine3f Renderer::RotateAroundPoint(Vector3f& axis, Vector3f& center, float theta)
{
	Affine3f A = Translation3f(center) * AngleAxisf(theta, axis) * Translation3f(-center);
	return A;
}
Renderer::~Renderer()
{
...
double Renderer::Picking(double newx, double newy)
{
		...
		bool picked = (igl::unproject_onto_mesh(Eigen::Vector2f(x, y), view,
			core().proj, core().viewport, scn->data().V, scn->data().F, fid, bc)) && (!scn->data().meshgl.is_cubemap);
...
IGL_INLINE void Renderer::resize(GLFWwindow* window,int w, int h)
	...
	//camera funcionality for the game - Eden Mansdorf

	/// <summary>
	/// Changes camera position to first person (snakes eye)
	/// </summary>
	...
	/// <summary>
	/// Changes camera position to the world view
	/// </summary>
