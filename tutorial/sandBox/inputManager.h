...
static void glfw_mouse_scroll(GLFWwindow* window, double x, double y)
{
	...
	if(rndr->IsPicked())
		...
		rndr->TranslateCamera(Eigen::Vector3f(0, -y * 0.03, 0));
}
...
static void glfw_key_callback(GLFWwindow* window, int key, int scancode, int action, int modifier)
{
	...
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		...
		{
		case GLFW_KEY_UP:
			scn->MoveInDirection(Direction::up);
			break;
		case GLFW_KEY_DOWN:
			scn->MoveInDirection(Direction::down);

			break;
		case GLFW_KEY_LEFT:
			scn->MoveInDirection(Direction::left);
			break;
		case GLFW_KEY_RIGHT:
			scn->MoveInDirection(Direction::right);
			break;
		case ' ':
